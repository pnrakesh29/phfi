...
function submitData() {
	...
	if (!validPermission(choices, 'errorDataPermission','Atleast one permission has to be chosen for role')
	     	| !checkValidRoles()
			| !validRoleDescription('description', 'roleDescription','Please enter the description','Please enter the valid description')
            | !validRoleName('roleName', 'roleDiv', 'Please enter the role name','Please enter the valid role name')
	) {
		return false;
	} else {
...
function checkValidEditRoles() {
	...
	if (!validRoleName('roleName', 'roleDiv', 'Please enter the role name', 'Please enter the valid role name')
			| !validRoleDescription('description', 'roleDescription', 'Please enter the description', 'Please enter the valid description')
			| !validPermission(choices, 'errorDiv', 'Atleast one permission has to be chosen for role')
			| !checkValidRoles()
...
function submitNext() {
	...
	if (!validRoleDescription('description', 'roleDescription', 'Please Enter the Description', 'Please Enter the Valid Description')
			| !validPermission(obj.length, 'errorData', 'Atleast one permission has to be chosen for role')
			| !checkValidEditRoles()) {
		return false;
	} else {
		return true;
...
function checkPermission() {
	...
	if (len < 1) {
		setDiv('errorData', 'Atleast one permission has to be chosen for role');
		return false;
	}
	setDiv('errorData', "&nbsp;");
	return true;
...
function submitEditData() {
	var obj = document.getElementsByClassName("checkboxCustom checkedCustom");
	if (!validPermission(obj.length, 'errorData', 'Atleast one permission has to be chosen for role')
			| !validRoleDescription('description', 'roleDescription', 'Please Enter the Description', 'Please Enter the Valid Description')
			| !validRoleName('roleName', 'roleDiv', 'Please enter the Role Name', 'Please enter the valid Role Name')) {
		return false;
	} else {
...
function confirmDeleteRole(roleId) {
	...
	var flag = confirm("Confirm To Delete Role");
	if (flag == true) {
