...
public class AdminProfileHandlerImpl implements AdminProfileHandler {
	...
	private void sendEmailNotification(final String toMail, final String FirstName, final String password) {
		try {
			...
			String subject = Properties.getProperty("phfi.email.admin.changepass.subject");
			String emailBody = readHtmlContent(Properties.getProperty("phfi.email.template.file.path") + File.separator + Properties.getProperty("phfi.email.customer.forgot.password.file"));

			String url = null;
			if (emailBody != null && !emailBody.equals("")) {
			    url = Properties.getProperty("phfi.admin.beacon.user.redirection.url");
