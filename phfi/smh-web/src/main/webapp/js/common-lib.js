...
function validatePopupDesc() {
	...
	// If so return true.
	if (isEmpty(text)) {
		setDiv("popDescError_div", "Please enter a Reason");
		return false;
	}
	if (!invalidWords(text)) {
		setDiv("popDescError_div",
				"A Word with 30 or more characters not allowed");
		return false;
	}
	if (!validMessage(text)) {
		setDiv("popDescError_div", "Please enter a valid Reason");
		return false;
	}
	if (text.length < 3) {
		setDiv("popDescError_div",
				"Reason should not be less than 3 characters");
		return false;
	} 
	if (! clientValidation('reason', 'reason','popDescError_div')) {
		setDiv("popDescError_div", "Please enter a valid Reason");
		return false;
	}
	else {
...
function downloadReports(curPageNumber, type) {
...
function checkMaxLength(textarea) {
	...
	document.getElementById('totalChars').innerHTML = textarea.value.length;
	document.getElementById('charsLeft').innerHTML = maxLength - textarea.value.length;
	return true;
}
